# NIH Grant Classifier — Reusable Workflow

## How This Works

When the user uploads a CSV batch of NIH grants to classify, follow this process:

### Phase 1: Run the Automated First-Pass Script
1. Copy the Python script from the `CLASSIFIER_SCRIPT` section below into `/home/claude/nih_grant_classifier.py`
2. Run it: `python3 /home/claude/nih_grant_classifier.py /mnt/user-data/uploads/<filename>.csv /home/claude/first_pass_<batchnum>.csv`
3. It outputs a CSV with a `review_flag` column

The script handles deterministic rules only:
- Activity code rules (training codes → training 95%, infrastructure codes → infrastructure 95%) - AUTO OK
- Multi-component grant core detection (P01/U19 admin cores etc.) - AUTO OK
- SBIR/STTR enforcement (always company, never basic_research)
- Org type classification
- Keyword-based content scoring - only AUTO OK if score ≥60, margin ≥30, confidence ≥90%

Grants flagged `OK` are VERY high-confidence automated classifications (requires score ≥60, margin ≥30, confidence ≥90%).
Only activity code deterministic rules (training/infrastructure) and overwhelming keyword matches skip review.
Grants flagged `REVIEW` need Claude to read and classify semantically. Goal: 90%+ projects at 80%+ confidence.

### Phase 2: Claude Semantic Review
This is where classification quality comes from. No rule-based script — Claude reads each REVIEW grant and applies expert judgment.

1. Run the helper: `python3 /home/claude/review_helper.py <batchnum>`
   Creates `/home/claude/review_chunks_<batchnum>/chunk_*.txt` files (~20 grants each with ID, activity code, org, title, full abstract up to 3000 chars).

2. Read each chunk using `view` tool. For each grant, determine the primary_category by asking: **"What is the primary deliverable of this project?"**

3. Write corrections to `/home/claude/corrections_<batchnum>.csv` with columns:
   `application_id,primary_category,category_confidence,secondary_category`
   Only include rows where you are CHANGING the first-pass classification or adjusting confidence. Rows not listed keep their first-pass values.

4. Run merge: `python3 /home/claude/merge_results.py <batchnum>`
   Combines first-pass OK rows + corrections into final output at `/mnt/user-data/outputs/classified_output_<batchnum>.csv`.

### Semantic Review Guidelines
When reading each REVIEW grant, apply these principles:

**Primary question: "What is this project building/producing?"**
- Knowledge about biology/mechanisms → basic_research
- A drug, vaccine, gene therapy, or treatment → therapeutics
- A tool, method, assay, or platform for researchers → biotools
- A clinical test or diagnostic → diagnostics
- A physical device for patients → medical_device
- Patient/clinician-facing software → digital_health
- Behavioral intervention, epi study, health services research → other

**Critical disambiguation (apply these rules carefully):**

- **USES vs DEVELOPS (most important)**:
  - "Using scRNA-seq to study tumors" = basic_research (tool is the means)
  - "Improving scRNA-seq methodology" = biotools (tool is the end)
  - Key question: Is the method/assay/platform the PRIMARY DELIVERABLE or just a technique being used?

- **Assay development distinction**:
  - "Develop an assay for detecting X" = biotools (creates research tool) OR diagnostics (if for clinical use)
  - "Use assay to measure X in patients" = basic_research (uses tool for knowledge)
  - Look for: "clinical validation", "clinical utility", "point-of-care" → diagnostics
  - Look for: "research community", "widely applicable", "resource" → biotools

- **Mechanism discussion ≠ basic_research**: Therapeutics abstracts discuss mechanisms. Look at what they're BUILDING.
- **Behavioral without drugs/devices = other**: CBT, exercise, cognitive training, mindfulness, peer-led prevention, rehabilitation (no drug/device) → other.
- **"Targeting X for treatment" = therapeutics**: Even if abstract is full of mechanism language.
- **Device trials = medical_device**: LVAD trials, neurostimulation, implant studies → medical_device, not basic_research.
- **SBIR = commercial**: Never basic_research. Usually therapeutics, medical_device, diagnostics, biotools, or digital_health.
- **Drug/antibiotic/vaccine DEVELOPMENT = therapeutics**, not biotools.
- **Epidemiology, cohort studies, health disparities = other**, even with biological measurements.
- **Biomarker intent**: Discovery = basic_research. Clinical validation = diagnostics.
...
def classify_project(row):
    ...
    # Requires: score >= 60, confidence >= 90, margin >= 30 (overwhelming signal)
    # OR activity code deterministic (handled earlier, returns before this)
    flag = 'OK' if (conf >= 90 and margin >= 30 and best_score >= 60) else 'REVIEW'
    return app_id, best_cat, conf, secondary, org_type, flag

def main():
    input_path = sys.argv[1]
    output_path = sys.argv[2] if len(sys.argv) > 2 else '/home/claude/classified_first_pass.csv'
    with open(input_path, 'r') as f: rows = list(csv.DictReader(f))
...
#!/usr/bin/env python3
"""
Extracts REVIEW-flagged grants into readable chunks for Claude semantic review.
Usage: python3 review_helper.py <batch_number>
"""
import csv, os, sys

def main():
    batch = sys.argv[1]
    source_csv = f'/mnt/user-data/uploads/semantic_batch_{batch}.csv'
    first_pass_csv = f'/home/claude/first_pass_{batch}.csv'
    chunk_dir = f'/home/claude/review_chunks_{batch}'
    os.makedirs(chunk_dir, exist_ok=True)

    with open(source_csv, 'r') as f:
        source = {r['application_id']: r for r in csv.DictReader(f)}
    with open(first_pass_csv, 'r') as f:
        first_pass = list(csv.DictReader(f))

    reviews = [fp for fp in first_pass if fp['review_flag'] == 'REVIEW']
    ok_count = sum(1 for fp in first_pass if fp['review_flag'] == 'OK')
    print(f"Batch {batch}: {len(first_pass)} total, {ok_count} OK, {len(reviews)} REVIEW")

    # Write summary CSV of all REVIEW items with first-pass suggestions
    summary_path = f'/home/claude/review_summary_{batch}.csv'
    with open(summary_path, 'w', newline='') as f:
        f.write('application_id,first_pass_category,first_pass_confidence,first_pass_secondary,org_type,activity_code,title\n')
        for fp in reviews:
            s = source[fp['application_id']]
            title = s['title'].replace('"', "'").replace(',', ';')[:120]
            f.write(f"{fp['application_id']},{fp['primary_category']},{fp['category_confidence']},{fp['secondary_category']},{fp['org_type']},{s['activity_code']},{title}\n")

    # Write chunks of ~20 grants each (with full 3000-char abstracts, this keeps chunks manageable)
    chunk_size = 20
    for i in range(0, len(reviews), chunk_size):
        chunk = reviews[i:i + chunk_size]
        chunk_num = i // chunk_size + 1
        chunk_path = f'{chunk_dir}/chunk_{chunk_num:02d}.txt'
        with open(chunk_path, 'w') as f:
            f.write(f"=== CHUNK {chunk_num} (grants {i+1}-{i+len(chunk)} of {len(reviews)}) ===\n")
            f.write(f"First-pass suggestion shown as [SUGGESTED: category, confidence]\n\n")
            for fp in chunk:
                aid = fp['application_id']
                s = source[aid]
                abstract = (s.get('abstract') or '').strip()
                if len(abstract) > 3000:
                    cutoff = abstract[:3000].rfind('.')
                    if cutoff > 2500: abstract = abstract[:cutoff + 1]
                    else: abstract = abstract[:3000] + '...'
                f.write(f"--- {aid} ---\n")
                f.write(f"Code: {s['activity_code']} | Org: {s['org_name'][:60]} | OrgType: {fp['org_type']}\n")
                f.write(f"[SUGGESTED: {fp['primary_category']}, {fp['category_confidence']}]\n")
                f.write(f"Title: {s['title']}\n")
                f.write(f"Abstract: {abstract}\n\n")
        print(f"  Wrote {chunk_path} ({len(chunk)} grants)")

    total_chunks = (len(reviews) + chunk_size - 1) // chunk_size
    print(f"\nCreated {total_chunks} chunks in {chunk_dir}/")
...
#!/usr/bin/env python3
"""
Merges first-pass OK results with Claude's semantic corrections.
Usage: python3 merge_results.py <batch_number>
"""
...
def main():
    batch = sys.argv[1]
    first_pass_csv = f'/home/claude/first_pass_{batch}.csv'
    corrections_csv = f'/home/claude/corrections_{batch}.csv'
    output_csv = f'/home/claude/classified_output_{batch}.csv'
    final_csv = f'/mnt/user-data/outputs/classified_output_{batch}.csv'
    ...
    corrections = {}
    try:
        with open(corrections_csv, 'r') as f:
            for row in csv.DictReader(f):
                corrections[row['application_id']] = row
        print(f"Loaded {len(corrections)} corrections")
    except FileNotFoundError:
        print("No corrections file found, using first-pass as-is")
    ...
    for fp in first_pass:
        ...
        if aid in corrections:
            c = corrections[aid]
            results.append((aid, c['primary_category'], int(c['category_confidence']),
                          c.get('secondary_category', ''), fp['org_type']))
        else:
            results.append((aid, fp['primary_category'], int(fp['category_confidence']),
                          fp['secondary_category'], fp['org_type']))

    with open(output_csv, 'w', newline='') as f:
        f.write('application_id,primary_category,category_confidence,secondary_category,org_type\n')
        for r in results:
            f.write(f'{r[0]},{r[1]},{r[2]},{r[3]},{r[4]}\n')
    shutil.copy2(output_csv, final_csv)

    cats = Counter(r[1] for r in results)
    review_count = sum(1 for fp in first_pass if fp['review_flag'] == 'REVIEW')
    corrected = sum(1 for fp in first_pass if fp['application_id'] in corrections)
    print(f"Final: {len(results)} records -> {final_csv}")
    print(f"Categories: {sorted(cats.items(), key=lambda x:-x[1])}")
    print(f"REVIEW: {review_count} total, {corrected} corrected, {review_count - corrected} kept")
