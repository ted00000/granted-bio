...
- **CRITICAL: DO NOT USE THE PHRASE "N ACTIVE OR COMPLETED" IN THE EXECUTIVE SUMMARY.** That label is ambiguous - readers disagree on whether "recruiting" counts as "active" or as its own state, and different readings produce different numbers. Instead cite each specific status individually if you cite trial status at all: "21 recruiting, 21 completed, 10 active-not-recruiting, 1 not yet recruiting, and 15 terminated/suspended/withdrawn (${totalTrialsForSummary} total)." That formulation reconciles unambiguously against the By Status table below. If you want a compact framing, say "${totalTrialsForSummary - trialStatusCounts.terminated} trials in progress, planned, or completed vs ${trialStatusCounts.terminated} terminated/suspended/withdrawn" - this uses the terminated bucket as the anchor because the negative counts are unambiguous. Do NOT attribute the terminated count to any specific cause ("assay attrition", "assay failure", "clinical failure") without corroborating evidence. If you mention terminations, frame as "worth monitoring" or "warrants investigation."
