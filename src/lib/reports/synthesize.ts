...
- **CRITICAL: DO NOT USE THE PHRASE "N ACTIVE OR COMPLETED" IN THE EXECUTIVE SUMMARY.** That label is ambiguous - readers disagree on whether "recruiting" counts as "active" or as its own state, and different readings produce different numbers. Instead cite each specific status individually if you cite trial status at all: "21 recruiting, 21 completed, 10 active-not-recruiting, 1 not yet recruiting, and 15 terminated/suspended/withdrawn (${totalTrialsForSummary} total)." That formulation reconciles unambiguously against the By Status table below.
- **STATUS ENUMERATION COMPLETENESS.** If you enumerate ANY status counts in the Exec Summary, you MUST enumerate ALL non-zero categories such that the cited counts sum to ${totalTrialsForSummary}. Do NOT cite "recruiting + completed + terminated/suspended/withdrawn" while silently dropping "not yet recruiting" and "active-not-recruiting" - a reader adding the cited counts will get less than ${totalTrialsForSummary} and lose confidence. If some category is 0, omit it silently; if it's >=1, cite it. Sanity check: sum every count you cite and confirm it equals ${totalTrialsForSummary} before writing the sentence.
- **NO NEGATION OF ABSENT CATEGORIES.** Do NOT write "with no trials in active-not-recruiting", "with no trials in not-yet-recruiting", or any "with no [X] status" negation. Just cite the status categories that are non-zero and stop - readers understand that unlisted categories are absent. Negation reads as reassurance but invites error: if you get the negation wrong (as happened when the LLM said "no active-not-recruiting" when there were 10), you've printed a lie. Cite what IS present; omit what isn't.
- **COMPACT ALTERNATIVE.** If you want a compact framing, say "${totalTrialsForSummary - trialStatusCounts.terminated} trials in progress, planned, or completed vs ${trialStatusCounts.terminated} terminated/suspended/withdrawn (${totalTrialsForSummary} total)" - this uses the terminated bucket as the anchor because the negative counts are unambiguous.
