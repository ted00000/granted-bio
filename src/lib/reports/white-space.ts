...
**TAXONOMY LABEL REQUIRED (Dimension 7).** Whenever you cite a project count or share in this narrative, you MUST name the dimension explicitly - the reader has to be able to tell which taxonomy your count comes from. Example: "48 projects (39.0%)" alone is ambiguous - could be a funding-category count from the Executive Summary or an analyte-class count from this dimension. WRITE INSTEAD: "48 projects (39.0%) in the [Dimension Name] dimension" or "48 projects in this dimension (39.0% of the sample)". The dimension name is the header of the section you're writing narrative for (e.g., "Analyte Class dimension", "Cancer Type dimension", "Detection Methodology dimension"). Every count must be attributable to its taxonomy source. This rule holds for share ratios ("2.5x sample-share") as well as absolute counts.

**CRITICAL — Do NOT draw underrepresentation or "gap" conclusions from broader-NIH counts below 30.** A category with (sample=0, broaderNIH=7) is too sparse to distinguish a real gap from vocabulary drift or keyword-matching noise. If you MUST mention such a category in the dimension narrative, describe the raw counts only ("population disparities appears in 0 projects with 7 broader NIH matches, too sparse to draw a coverage conclusion") — do NOT write "underrepresented", "gap", or "opportunity" for those rows. The Coverage Gap Signals list is where opportunity-language belongs, and it applies its own >=30 floor. This constraint applies to strategicImplications TOO - do not surface sub-30 categories as "strongest differentiation opportunities" or "least-covered intersections." Only the ranked opportunities passed to you in the RANKED WHITE SPACE OPPORTUNITIES section are eligible for that framing.

**CRITICAL — NEVER SUM NON-EXCLUSIVE CATEGORY COUNTS.** Rows within a dimension are non-exclusive: a project can appear in multiple categories. If category A has 31 projects and category B has 23, you MUST NOT write "A and B collectively represent 54 projects" or "collectively 44% of the sample" - those projects overlap, so 31+23 double-counts. Similarly, do NOT compute "top 3 categories hold N/M matched projects" where N is a sum across categories - it's arithmetically wrong. Rules:
- To describe "how much of the sample" a category represents, cite ONE category's count and its share (e.g. "cfDNA/ctDNA appears in 31 of ${totalProjects} projects, 25.2%").
- To describe multiple leading categories, cite each separately ("cfDNA at 25.2%, methylation at 18.7%, EV proteomics at 16.3% - rows overlap").
- Never write "collectively represent X% of the sample" from a sum of non-exclusive row counts. This is the single most common statistical mistake in coverage narratives and readers spot it immediately.
